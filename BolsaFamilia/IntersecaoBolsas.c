...
int main (int argc, char ** argv)
{
	...
	FILE *f= fopen ("BolsaFamilia2.csv", "r");
	FILE *file= fopen ("intersecaoBolsas.csv", "w");
 	char linha[2048];
    char nis[12]; 
    int numColuna= 0;
    long posicaoArq;
    char *campo;
    fprintf (file, "=========================ARQUIVO INTERSEÇÃO=========================");
    fprintf (file, "Ano/Mês Referência\tAno/Mês Competência\tUF\tCódigo Município SIAFI\tNome Município SIAFI\tNIS Beneficiário\tNome Beneficiário\tValor Benefício");
    fgets (linha, 2048, f); 
    ftell (f);
    fgets (linha, 2048, f);
    while (!feof (f)) 
    {
        ...
        while (campo != NULL)
        {
            ...
            {
                strcpy (nis, campo);
                if (ArvoreB_Busca (a, nis) > -1)
                {
                    fseek (f, posicaoArq, SEEK_SET);
                    fprintf (file, "\n%s", fgets (linha, 2048, f));
                }
            }
            numColuna++;
            campo= strtok (NULL, ";");
